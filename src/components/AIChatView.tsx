...
export const AIChatView: React.FC<AIChatViewProps> = ({
  ...
  const preset = AI_PROVIDER_PRESETS[provider] || AI_PROVIDER_PRESETS.gemini;
  ...
  return (
    <div className="bg-white/95 dark:bg-oriental-dark-card/95 rounded-2xl sm:rounded-3xl shadow-2xl border-2 border-amber-200/90 dark:border-oriental-dark-border p-2.5 sm:p-6 backdrop-blur-md flex flex-col h-[calc(100vh-130px)] sm:h-[calc(100vh-150px)] max-h-[850px] transition-all relative">
      ...
      {selectedDayContext && (
        <div className="bg-gradient-to-r from-amber-200/90 via-amber-100/95 to-amber-200/90 dark:from-oriental-dark-bg dark:to-amber-950/60 p-2 sm:p-2.5 rounded-xl border-2 border-oriental-gold-500/50 text-[11px] sm:text-xs shadow-2xs flex items-center justify-between mb-2.5 shrink-0">
          <div className="flex items-center space-x-1.5 truncate">
            <span className="bg-oriental-red-800 text-oriental-gold-300 px-1.5 py-0.5 rounded-md text-[10px] font-bold flex items-center gap-1 shrink-0">
              <Pin className="w-2.5 h-2.5 fill-current" /> Ngữ cảnh
            </span>
          ...
          <button
            ...
            className="p-1 rounded-md text-amber-800 dark:text-amber-300 hover:bg-amber-300/60 dark:hover:bg-amber-900/60 shrink-0 ml-1 transition-colors"
      ...
      <div className="flex-1 overflow-y-auto space-y-3.5 pr-0.5 custom-scrollbar text-xs sm:text-sm">
        ...
        {messages.length === 0 && (
          <div className="bg-gradient-to-br from-amber-50/90 via-white to-amber-50/80 dark:from-oriental-dark-bg/90 dark:to-oriental-dark-card p-4 sm:p-7 rounded-2xl sm:rounded-3xl border-2 border-amber-200/80 dark:border-oriental-dark-border text-center my-auto shadow-sm">
            ...
            <p className="text-xs sm:text-sm text-amber-900/85 dark:text-amber-200/80 max-w-lg mx-auto leading-relaxed mb-4">
              Tôi là Trợ Lý AI của <strong>An Lịch AI</strong> ({preset.name}). Tôi sẵn sàng tư vấn xem ngày cát tường, hoàng đạo, tử vi 12 con giáp, phong thủy bát trạch và giờ xuất hành theo phương châm <strong>"Xem ngày • Hiểu mình • Sống an"</strong>.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-left max-w-lg mx-auto">
              <button
                ...
                className="p-2.5 sm:p-3 rounded-xl bg-gradient-to-r from-amber-100/90 to-amber-200/80 dark:from-oriental-dark-card dark:to-amber-950/40 hover:brightness-105 border border-oriental-gold-500/40 text-amber-950 dark:text-amber-200 font-bold text-xs transition-all flex items-center space-x-2 shadow-2xs active:scale-98 cursor-pointer"
              >
              ...
              <button
                ...
                className="p-2.5 sm:p-3 rounded-xl bg-gradient-to-r from-amber-100/90 to-amber-200/80 dark:from-oriental-dark-card dark:to-amber-950/40 hover:brightness-105 border border-oriental-gold-500/40 text-amber-950 dark:text-amber-200 font-bold text-xs transition-all flex items-center space-x-2 shadow-2xs active:scale-98 cursor-pointer"
              >
                <Compass className="w-4 h-4 text-oriental-gold-600 shrink-0" />
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            <div
              className={`max-w-[92%] sm:max-w-[83%] rounded-2xl shadow-md leading-relaxed ${
                msg.role === 'user'
                  ? 'bg-gradient-to-r from-oriental-red-800 via-oriental-red-900 to-oriental-red-950 text-amber-50 font-semibold rounded-tr-xs p-3 sm:p-4 border border-oriental-gold-400/40 text-xs sm:text-sm'
                  : 'bg-white/95 dark:bg-oriental-dark-card/95 border-2 border-amber-200/90 dark:border-oriental-dark-border text-slate-800 dark:text-amber-100 rounded-tl-xs p-3.5 sm:p-5'
              }`}
            >
              {msg.role === 'user' ? (
        ...
        {isLoading && !currentStreamText && (
          <div className="flex items-center space-x-2 text-slate-700 dark:text-amber-200 font-bold text-xs py-2 px-3.5 bg-amber-100/80 dark:bg-oriental-dark-card rounded-xl border-2 border-oriental-gold-500/40 w-fit shadow-2xs">
            <RefreshCw className="w-3.5 h-3.5 animate-spin text-oriental-gold-600" />
      ...
      {/* Input Form & Mobile Optimized Suggestion Pills */}
      <div className="pt-2.5 border-t border-amber-200/80 dark:border-oriental-dark-border space-y-2 shrink-0">
        
        {/* Quick Suggestion Pills Bar - Mobile Friendly Touch Scroll */}
        <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pb-0.5 -mx-1 px-1">
          <button
            type="button"
            onClick={() => handleSend('Hãy luận tử vi 12 con giáp chi tiết cho ngày hôm nay')}
            className="px-2.5 py-1 rounded-full bg-gradient-to-r from-amber-100/90 to-amber-200/80 dark:from-amber-950/60 dark:to-oriental-dark-card text-amber-950 dark:text-amber-200 font-bold text-[11px] sm:text-xs whitespace-nowrap transition-all border border-oriental-gold-500/50 shadow-2xs flex items-center gap-1 active:scale-95 cursor-pointer shrink-0"
          >
            <Star className="w-3 h-3 text-amber-500 fill-current" />
            <span>Tử Vi 12 Con Giáp</span>
          </button>
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-full bg-gradient-to-r from-amber-100/90 to-amber-200/80 dark:from-amber-950/60 dark:to-oriental-dark-card text-amber-950 dark:text-amber-200 font-bold text-[11px] sm:text-xs whitespace-nowrap transition-all border border-oriental-gold-500/50 shadow-2xs flex items-center gap-1 active:scale-95 cursor-pointer shrink-0"
          >
            <Compass className="w-3 h-3 text-oriental-gold-600" />
          ...
          <button
            ...
            className="px-2.5 py-1 rounded-full bg-gradient-to-r from-amber-100/90 to-amber-200/80 dark:from-amber-950/60 dark:to-oriental-dark-card text-amber-950 dark:text-amber-200 font-bold text-[11px] sm:text-xs whitespace-nowrap transition-all border border-oriental-gold-500/50 shadow-2xs flex items-center gap-1 active:scale-95 cursor-pointer shrink-0"
          >
            <Sparkles className="w-3 h-3 text-oriental-gold-500" />
            <span>Nên Làm & Kiêng Kỵ</span>
          </button>
        </div>
...
function renderFormattedContent(text: string) {
  ...
  return (
    <div className="space-y-2.5 leading-relaxed text-xs sm:text-sm font-sans">
      {lines.map((line, idx) => {
        let trimmed = line.trim();
        if (!trimmed) return <div key={idx} className="h-0.5" />;

        // Strip markdown headers like ### 1. or ##
        const headerMatch = trimmed.match(/^(#{1,6})\s*(.*)/);
        if (headerMatch) {
          const headerTitle = cleanMarkdownText(headerMatch[2]);
          return (
            <div
              key={idx}
              className="mt-3 mb-1.5 pt-2 border-t border-amber-200/60 dark:border-oriental-dark-border flex items-center space-x-2 font-serif font-black text-oriental-red-900 dark:text-oriental-gold-400 text-xs sm:text-sm tracking-wide"
            >
              <span className="w-2 h-2 rounded-full bg-oriental-gold-500 shrink-0" />
              <span className="flex-1">{renderRichInlineText(headerTitle)}</span>
            </div>
          );
        }

        // Numbered item like "1. Giờ tốt xuất hành:"
        const matchNumbered = trimmed.match(/^(\d+)\.\s*(.*)/);
        if (matchNumbered) {
          const bodyText = renderRichInlineText(matchNumbered[2]);
          return (
            <div key={idx} className="mt-2 mb-1 font-serif font-bold text-oriental-red-900 dark:text-oriental-gold-400 text-xs sm:text-sm flex items-start gap-1.5">
              <span className="w-4.5 h-4.5 rounded-full bg-oriental-red-800 text-oriental-gold-300 text-[10px] font-mono font-bold flex items-center justify-center shrink-0 mt-0.5 shadow-2xs border border-oriental-gold-500/40">
                {matchNumbered[1]}
              </span>
              <span className="flex-1 leading-snug">{bodyText}</span>
            </div>
          );
        }
        ...
        if (trimmed.startsWith('- ') || trimmed.startsWith('* ') || trimmed.startsWith('• ') || trimmed.startsWith('✦ ')) {
          ...
          const bodyText = renderRichInlineText(bulletText);
          return (
            <div key={idx} className="ml-2 sm:ml-4 flex items-start gap-1.5 text-slate-700 dark:text-amber-200/90 text-xs sm:text-sm">
              <span className="text-oriental-gold-600 dark:text-oriental-gold-400 font-bold shrink-0 text-[10px] mt-0.5">✦</span>
              <span className="flex-1 leading-relaxed">{bodyText}</span>
            </div>
          );
        }

        // Normal paragraph text with bold inline formatting
        return (
          <p key={idx} className="text-slate-800 dark:text-amber-100 text-xs sm:text-sm leading-relaxed">
            {renderRichInlineText(trimmed)}
          </p>
        );
      })}
    </div>
